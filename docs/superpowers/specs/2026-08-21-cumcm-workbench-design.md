...
人工门未确认时，总控 Skill 必须停留在当前阶段。文献分支只形成候选文献，候选与拟支持主张随论文提纲在人工门 3 一并人工确认；这不是第五个全局人工门。确认记录需包含选择、理由、时间和对应产物版本。
...
| 文献研究（按需） | 待证据化主张、可用检索后端或用户文件 | 候选文献、人工决定、来源与引用记录 | 未批准候选不得进入正式引用，不得伪造元数据 |
...
| `literature-researcher` | 按需规划检索、归一化候选文献并形成可审核来源记录 | 无可用后端、元数据冲突、来源不可访问、全文不匹配、用户未确认或主张超出来源证据 |
...
- `cumcm-paper` 保留为 CUMCM 单 Skill 兼容入口，仅在用户显式选择时使用；它不是共享核心的开发源，也不得把缺失的 `cumcm_*` 工具写成可用。
- `math-modeling-paper` 保留为 MCM/ICM、其他建模竞赛或用户明确要求单 Skill 流程时的兼容入口；它不替代未来默认的 `cumcm-orchestrator`。
- 两项 legacy Skill 的资料和模板只能在来源复核、哈希清单、去重与测试通过后迁入 `shared/`，不得直接复制为 Codex 或 DSH 发布资产。
- `cumcm-orchestrator` 与 `literature-researcher` 均为后续阶段规划能力，本文不表示它们已安装或可运行。
...
核心 Skill 按路线图分批交付：阶段 3 先实现 `problem-reader`、`data-auditor`、`model-selector`、`solver`、`sensitivity-analyst` 和按需 `literature-researcher`；阶段 4 实现 `paper-outliner`、`paper-writer` 和 `latex-publisher`；阶段 5 实现 `model-reviewer` 与 `paper-reviewer`；阶段 6 最后接入 `cumcm-orchestrator`。其中 `paper-outliner` 必须在第三个人工确认门启用前完成。其他子 Skill 在对应阶段按实际瓶颈补充，不阻塞首个完整里程碑。
...
2. 契约与快照测试：工具 JSON、实验清单、`literature-source`、`citation-link`、LaTeX 骨架、错误格式和 Skill 触发边界。
3. 集成场景：评价、预测和优化三类代表场景，以及失败恢复和双端一致性。
4. 真题端到端：从赛题与数据运行到审批通过的 PDF。

引用负面场景必须覆盖无后端、元数据冲突、重复标识、全文与元数据不匹配、未获人工确认的候选、缺失定位和主张超出来源支持边界；这些场景均应失败关闭，不得伪造或静默插入引用。
...
| Phase 0A 论文与文献整合底座 | 文献来源与引用契约、受控路由和能力状态文档 | 后续 Phase 3/4/6/7 共享同一论文与文献策略 |
| 1 可复现底座 | 环境检查、项目骨架、实验清单、LaTeX 最小链路 | 新环境能诊断并编译最小 PDF |
| 2 高频模型核心 | 基础知识、首批模型卡、数据与实验工具 | 代表性数据能完成审计、基线与敏感性 |
| 3 Codex 子 Skill | 读题、审计、选模、求解、验证和按需文献研究 | 跑通可追溯建模半链路与候选文献路由 |
| 4 论文生产线 | 提纲、正文、摘要、图表、实验与引用证据绑定和 PDF | 生成不含无来源数值或引用的可编译论文 |
| 5 独立审批 | 复现、模型、论文、质询和提交审计 | 形成证据化问题报告和修订闭环 |
| 6 总控闭环 | 四个人工门、状态恢复和 72 小时流程 | 完成从赛题到审批 PDF 的完整流程 |
| 7 DSH 适配 | 同名 Skill、确定性搜索/读取工具插件、权限配置和真实组合测试 | 两端契约、调用和产物语义一致 |
| 8 真题回归 | 三类代表场景、历年真题与引用 provenance 复核 | 回归稳定后再扩展模型覆盖 |

Phase 0A 插入阶段 0 与阶段 1 之间而不重编号原有阶段；阶段 1 依赖 Phase 0A。阶段 0–3（含 Phase 0A）构成首个可用里程碑；阶段 0–6（含 Phase 0A）构成首个完整里程碑。DSH 契约从阶段 0 纳入设计，实际适配在核心工具和 Codex 流程稳定后进行。
...
| 工程范围失控 | 延迟形成可用能力 | 首批只做路线图明确归属的高频核心 Skill |
...
仓库已经初始化，阶段 0 契约基线已落地；截至提交 `86ea7e1`，目录登记 11 项契约。当前实施入口是 Phase 0A 论文与文献整合底座；后续阶段仍须把每个任务细化为具体文件、验证命令、验收证据和依赖关系，并遵守阶段依赖。
