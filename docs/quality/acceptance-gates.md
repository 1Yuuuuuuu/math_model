...
> **通过条件。** 本阶段只能在全部契约测试、离线验证与未完成标记扫描均成功后交付。验证过程离线、只读、零写入：不得联网，也不得修改被校验的契约或样例文件。
...
验证器使用本地 [catalog.json](../../shared/contracts/catalog.json) 和本地 Schema，不解析远程引用，也不下载规则或依赖。运行结束后，应比较文件清单或执行 `git status --short`，确认没有新增字节码、缓存或被改写的样例。年度规则的网络核验是更新规则时的独立人工步骤，不属于这里的离线验证。
