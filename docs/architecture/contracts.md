...
| 文件路径 | 相对工作区、使用正斜杠 `/`、不可越界；不得使用盘符、反斜杠、绝对路径或 `..` | `data/input.csv` 合法；`../secret.csv` 不合法 |
| 时间 | 所有时间都必须是带时区的 RFC 3339 字符串 | `2026-09-10T10:15:00+08:00` |
| 哈希 | SHA-256 为 64 位小写十六进制 | `aaaaaaaa...`（共 64 位） |

`scripts/contract_formats.py` 提供一致的 RFC 3339、路径关联时间，以及 HTTP(S) 来源 URL 解释。Codex 与 DeepSeek Harness 的实现都应使用这一格式检查器或等价实现；默认校验器若跳过格式检查，会放过不带时区的时间或不合规 URL。
