...
/** Object or JSON null. */
const nullableObject = { oneOf: [openObject, { type: 'null' }] };
...
const SEARCH_BACKENDS = ['paper-search', 'runtime-search', 'user-provided'];
 ...
 */
async function resolvePython(config) {
 ...
 * an editable install.
 */
function buildPythonPath(sourceRoot) {
 ...
 */
async function runRulesCli(config, args, signal) {
    ...
    try {
        ...
        if (outcome.code === 0) {
            if (jsonError !== null) {
                ...
                return {
                    ok: false,
                    error: `literature rules CLI: ${jsonError}${stderrFirst ? ` (stderr: ${stderrFirst})` : ''}`,
                };
            }
...
function pushGap(gaps, field, value) {
 ...
 * `metadata_gaps`.
 */
function buildCandidateFromJson(parsed) {
    ...
    if (isRecord(identifiers)) {
        const mapped = {};
        for (const key of ['doi', 'arxiv_id', 'pmid']) {
            const value = identifiers[key];
            if (typeof value === 'string' && value !== '')
                mapped[key] = value;
        }
        if (Object.keys(mapped).length > 0)
            out.identifiers = mapped;
    }
    else {
        const flatDoi = pick('doi');
        if (typeof flatDoi === 'string' && flatDoi !== '')
            out.identifiers = { doi: flatDoi };
    }
    ...
    if (typeof retrievedAt === 'string' && retrievedAt !== '')
        out.retrieved_at = retrievedAt;
    const backend = pick('retrieval_backend');
    if (typeof backend === 'string' &&
        SEARCH_BACKENDS.includes(backend)) {
...
function extractPdfText(buf) {
    ...
    return text
        ? {
            text,
            note: 'pdf minimal text extraction (uncompressed Tj streams only); metadata fields not populated',
        }
        : {
            text: null,
            note: 'pdf text unavailable via plain-text fallback (compressed or scanned); metadata fields not populated',
        };
}
...
async function readSource(filePath) {
    ...
    try {
        ...
        throw new Error(`literature_read_source: cannot read ${filePath}: ${error instanceof Error ? error.message : String(error)}`);
    }
    ...
    if (extension === '.jsonl') {
        throw new Error('literature_read_source: unsupported .jsonl input — only single-object .json files are supported');
    }
    if (extension === '.json') {
        ...
        try {
            ...
            throw new Error(`literature_read_source: invalid JSON in ${filePath}: ${error instanceof Error ? error.message : String(error)}`);
        }
...
const SOURCE_ROOT_NOTE = '前置条件：sourceRoot 必须指向 cumcm-workbench 仓库根（含 toolkit/src/cumcm_toolkit），' +
...
const FAIL_CLOSED_NOTE = '失败语义：子进程非 0 退出、stdout 非 JSON 或契约不符时，工具返回 failed 并附明确 error，绝不返回伪造结果。';
const CANDIDATE_NOTE = '候选 ≠ 引用（Phase 0A 政策）：本工具只产出/路由 candidate，未经人工核验不得作为正式引用。';
function buildTools(config) {
    return [
        defineTool({
            name: 'literature_read_source',
            description: '离线确定性解析单个文献源文件（PDF/单对象 JSON/纯文本），输出与 literature-source 契约 candidate 状态兼容的候选对象。' +
        ...
        defineTool({
            ...
            description: '按共享去重规则（shared/knowledge/literature/deduplication.md，参考实现 tests/knowledge/test_literature_knowledge.py）' +
            ...
            async execute(args, exec) {
                if (args.candidate.length > MAX_CANDIDATE_ARGV_CHARS) {
                    throw new Error('literature_route_candidate: candidate payload too large — split into batches of ~500 records');
                }
                ...
                if (!isRecord(data) || !isRecord(data.groups) || !isRecord(data.conflicts)) {
                    throw new Error('literature_route_candidate: rules CLI output missing groups/conflicts');
                }
                return data;
            },
        }),
        defineTool({
            ...
            description: '文献检索——仅当配置了 backend 且其标识在 allowedDomains 白名单内时可用；否则 blocked（明确错误，不静默降级）。' +
            ...
            async execute(args) {
                ...
                if (!allowed.includes(backend)) {
                    throw new Error(`literature_search: domain not allowed — backend "${backend}" is not covered by allowedDomains`);
                }
                return {
                    backend,
                    query: args.query,
...
export function apply(ctx, config) {
    ...
    if (sourceRoot === '') {
        throw new Error('literature-tools: sourceRoot is required — point it at the cumcm-workbench repo root ' +
