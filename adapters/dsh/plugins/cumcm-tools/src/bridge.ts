 ...
 * matches.
 */
export async function resolvePython(config: CumcmToolConfig): Promise<string[]> {
  ...
  if (await commandOnPath('uv')) return ['uv', 'run']
  throw new Error(
    'cannot resolve a python interpreter: pythonBin is empty, ' +
 ...
 */
function buildPythonPath(cwd: string): string {
...
export async function runPythonTool(
  ...
  const cwd =
    config.cumcmRoot && config.cumcmRoot.trim() !== ''
      ? config.cumcmRoot
      : options.cwd || process.cwd()
  const timeoutMs =
    Number.isFinite(config.toolTimeoutMs) && config.toolTimeoutMs > 0
  ...
  try {
    ...
    if (outcome.code === 0) {
      if (jsonError !== null) {
        ...
        return {
          ok: false,
          error: `cumcm_toolkit.${moduleName}: ${jsonError}${
