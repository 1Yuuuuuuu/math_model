...
import { runPythonTool, type CumcmToolConfig } from './bridge.js'
...
const PREREQ_NOTE =
  '前置条件：插件已配置 cumcmRoot（cumcm-workbench 仓库根，含 .venv 与 toolkit/src）或 pythonBin 指向可用 python；' +
  '子进程以 cumcmRoot 为 cwd、注入 PYTHONPATH=<cumcmRoot>/toolkit/src;<cumcmRoot> 执行 `python -m cumcm_toolkit.*`。'
const FAIL_CLOSED_NOTE =
  '失败语义：Python CLI 子进程非 0 退出、stdout 非 JSON 或参数契约不符时，工具返回 failed 并附明确 error，绝不返回伪造结果。'
...
function buildTools(config: CumcmToolConfig) {
  return [
    defineTool({
      name: 'cumcm_data_profile',
      description:
        '对 CSV 数据文件做画像分析，返回结构化 profile 报告（行/列数、每列 dtype/缺失/唯一值、数值列 min/max/mean/std、重复行、key 列唯一性、警告列表）。' +
        PREREQ_NOTE +
      ...
      async execute(args, exec) {
        ...
        if (args.key_columns !== undefined) argv.push('--key-columns', args.key_columns)
    ...
    defineTool({
      ...
      description:
        '按 JSON steps 对 CSV 应用数据变换（drop_columns/drop_missing/fill_missing/normalize/to_datetime/cast），写出到 output，返回 {steps_applied, warnings}。' +
        PREREQ_NOTE +
      ...
      async execute(args, exec) {
        return bridgeOk(config, 'cumcm_data_transform', 'data.transform',
          ['--input', args.input, '--steps', args.steps, '--output', args.output], exec.signal)
      },
    }),
    ...
    defineTool({
      ...
      description:
        '拟合注册的机器学习模型并返回 {status, model, params, seed, fitted}。' +
        PREREQ_NOTE +
      ...
      async execute(args, exec) {
        ...
        if (args.params !== undefined) argv.push('--params', args.params)
    ...
    defineTool({
      ...
      async execute(args, exec) {
        ...
        if (args.positive_label !== undefined) argv.push('--positive-label', args.positive_label)
    ...
    defineTool({
      ...
      description:
        '校验灵敏度报告输入契约（base_params 数值对象 + perturb 数值列表对象）并返回 {status, valid}。CLI 只做契约校验、不做评估。' +
        PREREQ_NOTE +
    ...
    defineTool({
      ...
      description:
        '为一条论断创建证据链接记录（evidence-link 契约，schema_version 1.0），返回校验通过的记录对象。' +
        PREREQ_NOTE +
      ...
      parameters: {
        claim: {
          ...
          required: true,
          description:
            'JSON 字符串：论断对象（含 claim_id/claim_text/artifact_id/experiment_id/locator/boundary）',
        },
      },
      output: {
        schema: {
    ...
    defineTool({
      ...
      description:
        '从已批准的文献源记录创建引用链接记录（citation-link 契约）并返回记录。源记录 verification_status 必须为 approved 且含 decision_id，否则失败关闭。' +
        PREREQ_NOTE +
    ...
    defineTool({
      ...
      description:
        '用本机 xelatex 编译论文目录（main.tex），有 bibliography.bib 时跑 bibtex，再跑 passes 次 xelatex，返回构建报告（status/errors/warnings/pages/undefined_references/failed_pass/pdf_path/log_path）。' +
        PREREQ_NOTE +
      ...
      async execute(args, exec) {
        ...
        if (args.passes !== undefined) argv.push('--passes', String(args.passes))
    ...
    defineTool({
      ...
      description:
        '对论文目录（main.tex + bibliography.bib）做静态 lint，返回 {status, issues}（issues 为 severity/kind/line/message 数组）。' +
        PREREQ_NOTE +
    ...
    defineTool({
      ...
      description:
        '检查 LaTeX 引用与 bibliography.bib、citation-link 记录、已批准源 id 的一致性，返回报告（status/missing_bibtex/unapproved_sources/uncited_entries/unmatched_citations/orphaned_citations/errors）。' +
        PREREQ_NOTE +
    ...
    defineTool({
      ...
      description:
        '检查 PDF 文件，返回 {status, pages, blank_pages, fonts, metadata, errors}。' +
        PREREQ_NOTE +
        '输入契约：pdf 为 PDF 文件路径。' +
        FAIL_CLOSED_NOTE,
      parameters: {
    ...
    defineTool({
      ...
      description:
        '把结果导出为文件（json/csv/latex 三选一），返回 {status, path, format}。' +
        PREREQ_NOTE +
      ...
      async execute(args, exec) {
        if (args.latex === true) {
          const argv = ['--latex']
          if (args.rows !== undefined) argv.push('--rows', args.rows)
          if (args.caption !== undefined) argv.push('--caption', args.caption)
    ...
    defineTool({
      ...
      description:
        '在 target 下按标准模板创建 CUMCM 工作区 <target>/<workspace_id>，返回 {workspace_id, root, files}（files 为 path/size/sha256 列表）。' +
        PREREQ_NOTE +
    ...
    defineTool({
      ...
      description:
        '创建实验清单记录（experiment.schema 契约，schema_version 1.0，要求 Python 3.11 与 project_root/uv.lock），返回记录对象。' +
        PREREQ_NOTE +
      ...
      async execute(args, exec) {
        ...
        if (args.project_root !== undefined) argv.push('--project-root', args.project_root)
        if (args.parameters !== undefined) argv.push('--parameters', args.parameters)
        if (args.random_seed !== undefined) argv.push('--random-seed', String(args.random_seed))
        if (args.output_artifacts !== undefined) argv.push('--output-artifacts', args.output_artifacts)
        if (args.metrics !== undefined) argv.push('--metrics', args.metrics)
    ...
    defineTool({
      ...
      description:
        '索引工作区产物，返回 artifact 记录数组（每条含 schema_version/artifact_id/kind/path/sha256/created_at/source_artifact_ids）。' +
        PREREQ_NOTE +
        '输入契约：root 为工作区根目录路径。' +
        FAIL_CLOSED_NOTE,
      parameters: {
...
export function apply(ctx: Context, config: Config): void {
  const normalized: CumcmToolConfig = {
