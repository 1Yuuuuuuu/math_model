...
## 输入与假设
输入为任意分布的数值或等级数据；假设样本独立（成对时用符号秩检验），不要求正态与方差齐性。

## 核心公式
Mann-Whitney U 基于秩和；Wilcoxon 符号秩检验对差值绝对值排秩；Kruskal-Wallis 为多组秩的 ANOVA 类比。
...
## 工具入口
scipy.stats.mannwhitneyu、scipy.stats.wilcoxon、scipy.stats.kruskal、scipy.stats.friedmanchisquare（配伍设计）。

## 最小示例
`from scipy.stats import mannwhitneyu; u, p = mannwhitneyu(a, b, alternative="two-sided")`。
