...
## 输入与假设
执行器支持独立两组的 Mann–Whitney U、配对两组的 Wilcoxon、至少两组独立样本的 Kruskal–Wallis，以及至少 2×2 的列联表卡方独立性检验。输入数组的叶节点必须是 JSON 原生的有限 `int`/`float`，布尔值、数值子类和字符串不会被 NumPy 隐式转为观测；Wilcoxon 样本必须等长，列联表计数必须非负且每行、每列总数为正。秩检验不要求正态，但仍要求与方法匹配的独立或配对设计。
