...
## 输入与假设
执行器支持 `one-sample-t`、`independent-t` 和 `paired-t`。输入数组的叶节点必须是 JSON 原生的有限 `int`/`float`，不会把布尔值、数值子类或字符串强制转换为观测；每个 t 检验样本至少 2 个观测。独立样本要求组内独立，配对样本要求一一对应且等长。t 检验假设样本均值或配对差值的抽样分布可由 t 分布近似；独立样本等方差版还要求方差齐性，Welch 版放宽该条件。
...
`alternative` 可取 `two-sided`、`less` 或 `greater`，缺省为双侧；独立样本的 `equal_variance` 缺省为 `false`。t 统计量、p 值和效应量在共同缩放后的稳定数值域中计算，因此对所有输入及单样本总体均值同时乘以同一正数保持不变，并避免极小有限数据的方差下溢。未知字段、非有限值、配对长度不等、真实零方差导致统计量或效应量不可定义时均失败关闭，不返回 NaN/Infinity。
